...
interface AJBBibleMapJourneyTextOne extends HTMLElement {
  journey: Journey;
  position: number;
  render: VoidFunction;
}
...
const ajbBibleMapJourneyTextOneElement: CustomElement<AJBBibleMapJourneyTextOne> =
  {
    journey: connect(scrollStore, scrollStoreJourneySelector),
    position: ({ journey }: AJBBibleMapJourneyTextOne): number => {
      const result = 0;
      if (journey?.type === 1) {
